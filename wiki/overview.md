...
[工作流概念比较](analyses/workflow-concepts-comparison.md) 和
...
| 当前 wiki 已包含 workflow 概念比较分析，以及 agent orchestration 与传统 workflow 的边界分析，并采用单一上游证据对象粒度的 source projections。 | [工作流概念比较](analyses/workflow-concepts-comparison.md)；[Agent Orchestration 与传统 Workflow 的边界](analyses/agent-orchestration-vs-workflow.md)；上方 source projection 示例。 | 本页只列入口示例，不维护完整页面索引。 |
