...
| wiki | [裸金属 Cluster Buildout 的 Process Manager 平台选型](analyses/bare-metal-cluster-buildout-process-manager-selection.md) | 当前 active analysis 示例，记录裸金属 buildout 场景中 Temporal 与 Airflow 的主 process manager 选型边界。 |
...
| wiki | [Temporal Child Workflows 文档](sources/temporal/child-workflows-docs.md)、[Airflow Deferrable Operators 文档](sources/apache-airflow/deferrable-operators-docs.md)、[DMTF Redfish Standards 页面](sources/dmtf/redfish-standards-page.md)、[OpenStack Ironic README](sources/openstack/ironic-readme.md)、[Slurm Overview 文档](sources/slurm/overview-docs.md) | 裸金属 buildout 选型分析新增的 source projection 示例，覆盖资源分区、等待/事件/人工输入和下层裸金属控制面。 |
...
| 裸金属 buildout 选型分析将 AI 调研草稿降权为线索，技术事实由 Temporal、Airflow 和裸金属控制面的一手 source projections 支撑。 | [裸金属 Cluster Buildout 的 Process Manager 平台选型](analyses/bare-metal-cluster-buildout-process-manager-selection.md)；上方裸金属 buildout source projection 示例。 | 本页不重复该分析的决策内容；具体条件、限制和 POC 边界见分析页。 |
